...
if(isset ($_POST['idsdate']) ) {
    ...
    try{
      ...
      if($sdate != '' ){
        ...
        $name= '';
        $sql = "SELECT * FROM `cashaccount` join customer on cashaccount.custid=customer.custid WHERE ldate='".$sdate."'";
                            ...
                            <div class="col-lg-12 grid-margin stretch-card">
                                <div class="card">
                                    <div class="card-body">
                                        ...
                                        <h4 class="card-title">Return Cash Payment </h4>
                                            ...
                                            <table id="example" class="table table-light table-bordered display table-sm" style="width:100%">
                                                ...
                                                <tbody>
                                                    <?php while ($row = $result->fetch_assoc()) { ?>
                                                        <tr>
                                                            <td><?= htmlspecialchars($row["ldate"]); ?></a></td>
                                                            <td><?= htmlspecialchars($row["balid"]); ?></a></td>
                                                            <td><?= htmlspecialchars($row["custname"]); ?></a></td>
                                                            <td width="200"><?= htmlspecialchars($row["detail"]); ?></a></td>
                                                            <td style="text-align:right;"><?= htmlspecialchars($row["amount"]); ?></td>
                                                            <td><button name="delete-cash" style="color:red" class="btn mdi-24px btn-rounded btn-sm btn-icon delete-cash mdi mdi-undo-variant" l-date="<?=$row["ldate"]?>" b-id="<?=$row["balid"]?>" c-id="<?=$row["custid"]?>"  ></button></td>
                                                        </tr>
                                                    <?php  } ?>
                                                </tbody>
                                            </table>
